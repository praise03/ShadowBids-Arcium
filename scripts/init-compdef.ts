...
const LUT = new PublicKey("AddressLookupTab1e1111111111111111111111111");
...
async function main() {
  ...
  const [lutAddr] = PublicKey.findProgramAddressSync(
    [mxePda.toBuffer(), lutSlot.toArrayLike(Buffer, "le", 8)], LUT,
  );

  console.log("Initing comp def with devnet IDL...");
  const tx = await prog.methods
    .initComputationDefinition(
      ...
      payer.publicKey, // finalization_authority
    )
    .accounts({
    ...
    .rpc();
  console.log("Comp def initialized! tx:", tx);
}
